...
- Intended package release: `trl==1.9.2.post12`
...
- explicit grammar-prefix preservation so fixed assistant scaffolds advance the
  XGrammar matcher before selected completion-token probabilities are scored;
...
2. build `trl==1.9.2.post12` from the immutable release commit;
