...
def test_liger_window_normalization_preserves_loss_and_gradient(loss_type, training, world_size):
    ...
    for active_tokens, global_micro_tokens in [(2, 6), (3, 6)]:
        ...
        def fused_loss(mask=mask, mean_micro_tokens=mean_micro_tokens, active_tokens=active_tokens, **kwargs):
            # Stand-in for Liger 0.8's documented microbatch mean. This checks
            # native trainer wiring, not GPU kernel correctness.
            assert torch.equal(kwargs["attention_mask"], mask)
            denominator = mean_micro_tokens if loss_type != "grpo" else active_tokens
